...
### npx (не требуется Rust toolchain)

Предварительно собранные бинарные файлы публикуются в npm, так что вы можете запустить `shirabe` одной командой — без `cargo build`, без установки Chrome :

```bash
npx @celestia-island/shirabe debug --port 3001
```

Корневой пакет `@celestia-island/shirabe` автоматически подтягивает правильный подпакет для вашей платформы (`-linux-x64` / `-darwin-arm64` / `-win32-x64`) через `optionalDependencies` + postinstall-селектор. Чтобы зафиксировать версию :
...
Для production-развёртываний MCP используйте **обёртку авто-перезапуска**, чтобы сервер оставался активным во время обновлений, не прерывая сессию клиента.
...
1. Обёртка запускает `shirabe mcp` в цикле `while true`.
2. Если процесс завершается, он перезапускается в течение 0.2 секунды.
3. Для обновления: `kill $(pgrep -f "shirabe mcp" | head -1)`
