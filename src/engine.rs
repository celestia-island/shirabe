...
pub(super) async fn spawn_browser(
                            ...
                            if let Some(entry) = buf.get_mut(id) {
                                if let Some(resp) = p.get("response") {
                                    entry.status = resp
                                        ...
                                        .and_then(|s| s.as_u64())
                                        .map(|s| s as u16);
                                    if let Some(mt) = resp.get("mimeType").and_then(|x| x.as_str())
                                    {
    ...
    let _ = tokio::time::timeout(init_timeout, client.command("Runtime.enable", json!({}))).await;
    let _ = tokio::time::timeout(init_timeout, client.command("Network.enable", json!({}))).await;
...
fn resolve_key(spec: &str) -> Result<KeySpec, String> {
    ...
    let (key, code, vk, text, printable): (String, String, u32, String, bool) = match name.as_str()
    {
